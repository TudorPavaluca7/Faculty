...
public class SelectCtrl {
    ...
    public void initialize() {
        ...
        IStmt ex3=new CompoundStatement(new AssignStatement("v",new ConstExpression(10)),new CompoundStatement(
                ...
                new PrintStatement(new VarExpression("v")),new PrintStatement(new ReadHeapExpression("a"))))));
        IStmt ex4=new CompoundStatement(new CompoundStatement(new CompoundStatement(new AssignStatement("a",new ConstExpression(1)),new AssignStatement("b",new ConstExpression(2))),new CompoundStatement(new AssignStatement("c",new ConstExpression(5)),new SwitchC(new ArithmeticExpression('*',new VarExpression("a"),new ConstExpression(10)),new ArithmeticExpression('*',new VarExpression("a"),new VarExpression("b")),new CompoundStatement(new PrintStatement(new VarExpression("a")),new PrintStatement(new VarExpression("b"))),new ConstExpression(10),new CompoundStatement(new PrintStatement(new ConstExpression(100)),new PrintStatement(new ConstExpression(300))),new PrintStatement(new ConstExpression(300))))),new PrintStatement(new ConstExpression(300)));
